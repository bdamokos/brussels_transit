...
def assign_variant_numbers(shapes: Dict[str, List[Dict]], monitored_stops: List[Dict[str, float]]) -> Dict[str, int]:
    """
    ...
    """
    def point_matches_stop(point: List[float], stop: Dict[str, float], threshold: float = 0.0001) -> bool:
        """Check if a point matches a stop location within threshold"""
        return abs(point[1] - stop['lat']) < threshold and abs(point[0] - stop['lon']) < threshold
    ...
    return {
        shape['shape_id']: idx + 1
        for idx, shape in enumerate(sorted_shapes)
    }

def load_shapes_from_gtfs(gtfs_dir: Path, route_id: str) -> Dict[str, List[List[float]]]:
    """
...
def get_route_variants(route_id: str, monitored_stops: List[Dict[str, float]], gtfs_dir: Path) -> Dict:
    """
    ...
    return {
        'variants': variants,
        'shape_to_variant': shape_to_variant
    }
