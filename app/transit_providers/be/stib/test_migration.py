...
def test_source_consistency():
    ...
    response = requests.post(f"{BASE_URL}/api/stib/stops", json=["8122", "8032"])
    if response.ok:
        data = response.json()
...
def test_all_endpoints():
    ...
    endpoints = {
        ...
        "get_stop_by_name": {
            "method": "GET",
            "url": "/api/stib/get_stop_by_name?name=roodebeek",
        },
        "get_nearest_stops": {
            "method": "GET",
            "url": "/api/stib/get_nearest_stops?lat=50.8466&lon=4.3528",
        },
        "search_stops": {
            "method": "GET",
            "url": "/api/stib/search_stops?query=roodebeek",
        },
        ...
        try:
            if endpoint["method"] == "GET":
                ...
                response = requests.post(
                    f"{BASE_URL}{endpoint['url']}", json=endpoint.get("data")
                )
...
def test_message_language():
    ...
    for lang in ["en", "fr", "nl"]:
        ...
        response = requests.get(
            f"{BASE_URL}/api/stib/messages", params={"preferred_language": lang}
        )
        if response.ok:
            data = response.json()
            ...
            if messages:
                ...
                if most_common_lang == lang:
                    ...
                    print(
                        f"  Available languages: {set(lang for lang, count in language_stats.items() if count > 0)}"
                    )
            else:
        ...
        response = requests.get(
            f"{BASE_URL}/api/stib/waiting_times", params={"preferred_language": lang}
        )
        if response.ok:
            data = response.json()
                    ...
                    for line in stop_data.get("lines", []):
                        if "message" in line:
                            ...
                            if "_metadata" in line and "language" in line["_metadata"]:
                                ...
                                print(
                                    f"  Provided language: {lang_meta.get('provided')}"
                                )
                                print(
                                    f"  Available languages: {lang_meta.get('available')}"
                                )
                                if lang_meta.get("warning"):
                                    print(f"  Warning: {lang_meta['warning']}")
                            else:
