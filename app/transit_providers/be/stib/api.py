...
def parse_service_message(message, stop_details):
    ...
    try:
        ...
        text, metadata = select_language(
            text_with_langs, provider_languages=AVAILABLE_LANGUAGES
        )
        if metadata["language"]["warning"]:
            logger.warning(
                f"Language warning for message: {metadata['language']['warning']}"
            )

        # Get affected stop names
        affected_stops = [
            stop_details.get(point["id"], {"name": point["id"]})["name"]
            for point in points
        ]
    ...
    try:
        ...
        async with await get_client() as client:
            ...
            if response.status_code != 200:
                logger.error(
                    f"Service messages API returned {response.status_code}: {response.text}"
                )
                return {"messages": []}
            ...
            for message in data.get("results", []):
                try:
                    ...
                    try:
                        ...
                        if stop_info:
                            stop_names = [
                                info.get("name", stop_id)
                                for stop_id, info in stop_info.items()
                            ]
                        else:
                            stop_names = (
                                affected_stop_ids  # Fallback to IDs if names not found
                            )
                    ...
                    is_monitored = (
                        bool(monitored_line_set)
                        and bool(set(affected_lines) & monitored_line_set)
                        or bool(monitored_stop_set)
                        and bool(set(affected_stop_ids) & monitored_stop_set)
                    )
...
async def get_route_colors(monitored_lines=None):
    ...
    routes_cache = {"timestamp": None, "data": {}}
    ...
    try:
        ...
        async with await get_client() as client:
            ...
            if not rate_limiter.can_make_request() or (
                ...
                logger.warning(
                    "Skipping vehicle positions due to rate limit constraints"
                )
                return {"vehicles": []}
            ...
            for record in data.get("results", []):
                try:
                    ...
                    for position in vehicle_positions:
                        ...
                        position_data = {"distance": distance, "next_stop": point_id}

                        positions[line_id][vehicle_direction].append(position_data)
                        logger.debug(
                            f"Added position data for line {line_id} ({vehicle_direction}): {position_data}"
                        )

                except (json.JSONDecodeError, KeyError, TypeError) as e:
                    logger.error(
                        f"Error processing vehicle position for line {line_id}: {e}"
                    )
                    continue
                ...
                for direction_id, vehicles_list in directions.items():
                    ...
                    variant = next(
                        (
                            ...
                            for v in variants
                            if v.stops
                            and v.stops[-1]["id"].rstrip("FG")
                            == direction_id.rstrip("FG")
                        ),
                        None,
                    )
                    ...
                    for vehicle_data in vehicles_list:
                        try:
                            ...
                            logger.error(
                                f"Error processing vehicle on line {line_id}: {e}"
                            )
                            continue
...
def convert_v2_to_v1_format(v2_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    v1_data = {
        ...
        "display_stops": v2_data.get(
            "display_stops", []
        ),  # Copy display stops directly
    }
        ...
        for variant in variants:
            ...
            if shape_coords:
                ...
                v1_variant = {
                    ...
                    "shape": [
                        [lat, lon] for lon, lat in shape_coords
                    ],  # Swap coordinates
    ...
    v1_data["messages"]["messages"] = [
        {
            ...
            "type": msg.get("type", ""),
            "is_monitored": (
                any(stop_id in monitored_stops for stop_id in msg["points"])
                and any(line in monitored_lines for line in msg["lines"])
            ),
        }
    ...
    for stop_id, stop_data in stops_data.items():
        if "metadata" in stop_data and stop_data["metadata"].get("warning"):
            v1_data["errors"].append(
                f"Stop {stop_id}: {stop_data['metadata']['warning']}"
            )
...
async def get_data(stop_id: Union[str, List[str]] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        )  # No need to pass monitored lines/stops here
